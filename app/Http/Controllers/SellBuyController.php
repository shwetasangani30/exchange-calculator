...
class SellBuyController extends Controller
{
    ...
    {
        $buy_sell_result = BuySellResult::where('user_id', Auth::id())->with('getSell')->whereHas('getSell')->orderBy('created_at', 'desc')->get();
        return view('buysell.sell.index', compact('buy_sell_result'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return Redirect::route('sellbuy.index')->with('success', 'Sell value has been added.');
    }
    ...
    public function show(string $id)
    {
        ...
        if ($buy_sell_result) {
            ...
            return view('buysell.sell.info', compact('buy_sell_result', 'buy_sell'));
        } else {
            return Redirect::route('sellbuy.index');
        }
    }
    ...
    {
        $buy_sell_result = BuySellResult::with('getSell')->whereRaw("FIND_IN_SET(" . $id . ", merged_ids)")->first();
        return view('buysell.sell.edit', compact('buy_sell_result'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($buy_sell) {
            ...
            if ($buy_sell_result->is_close_added == 1) {
                ...
                $difference = $total_average - $buy_sell_result->close;
                $total_close_value = $buy_sell_result->close * $buy_sell_result->total_quantity;
                $profit_loss = $buy_sell_result->total_value - $total_close_value;
            ...
            return Redirect::route('sellbuy.show', $buy_sell_result->buy_sell_id)->with('success', 'Data updated successfully');
        } else {
            return Redirect::route('sellbuy.index')->with('error', 'There is some problem in saving data');
        }
        return Redirect::route('sellbuy.index')->with('error', 'validation failed');
    }
    ...
    public function destroy(string $id)
    {
        ...
        if ($buy_sell) {
            ...
            if ($buy_sell_result) {
                ...
                $buy_sell_result->save();
                return Redirect::route('sellbuy.show', $buy_sell_result->buy_sell_id)->with('success', 'Deleted successfully');
            } else {
                return Redirect::route('sellbuy.index')->with('error', 'There is some problem in delete');
            }
        } else {
            return Redirect::route('sellbuy.index')->with('error', 'There is some problem in delete');
        }
        ...
        if ($buy_sell_result) {
            ...
            if (count($buy_sell_all)) {
                ...
                if ($buy_sell_result) {
                    ...
                    $difference = $buy_sell_result->total_average - $close_value;
                    ...
                    return Redirect::route('sellbuy.index')->with('error', 'Something went wrong!');
                }
            } else {
                return Redirect::route('sellbuy.index')->with('error', 'No any sell or buy value found!');
            }
        } else {
            return Redirect::route('sellbuy.index')->with('error', 'No any sell or buy value found!');
        }
     ...
     * add close value
     */
    public function buyClose(Request $request)
    {
        $request->validate([
            'close_val' => ['required', 'numeric'],
        ]);
        ...
        if ($buy_sell) {
            ...
            if (count($buy_sell_all)) {
                ...
                if ($buy_sell_result) {
                    ...
                    $buy_sell_result->save();
                    return Redirect::route('sellbuy.index')->with('success', 'Close value added successfully.');
                } else {
                    return Redirect::route('sellbuy.index')->with('error', 'Something went wrong!');
                }
            } else {
    ...
    {
        return Excel::download(new SellBuyExport(), 'sellandbuy.xlsx');
    }
}
